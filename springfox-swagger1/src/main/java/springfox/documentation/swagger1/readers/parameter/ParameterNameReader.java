...
public class ParameterNameReader implements ParameterBuilderPlugin {
  ...
  public void apply(ParameterContext context) {
    ...
    String paramType = parameterType(context, methodParameter);
    if (apiParam.isPresent() && hasText(apiParam.get().name())) {
      context.parameterBuilder().name(maybeOverrideName(apiParam.get().name(), paramType));
    }
  }
  ...
  @VisibleForTesting
  String parameterType(ParameterContext context, MethodParameter methodParameter) {
    ...
    return findParameterType(methodParameter, parameterType);
  }
